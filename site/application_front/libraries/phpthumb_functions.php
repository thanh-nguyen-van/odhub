...
class CI_phpthumb_functions {
    ...
    function version_compare_replacement($version1, $version2, $operator='') {
        ...
        for ($i = 0; $i < $parts_count; $i++) {
            $comparison = CI_phpthumb_functions::version_compare_replacement_sub($version1, $version2, $operator);
            if ($comparison != 0) {
                return $comparison;
            }
        }
    ...
    function exif_info() {
        ...
        if (empty($exif_info)) {
            ...
            $phpinfo_array = CI_phpthumb_functions::phpinfo_array();
    ...
    function ImageColorAllocateAlphaSafe(&$gdimg_hexcolorallocate, $R, $G, $B, $alpha=false) {
        if (CI_phpthumb_functions::version_compare_replacement(phpversion(), '4.3.2', '>=') && ($alpha !== false)) {
    ...
    function ImageHexColorAllocate(&$gdimg_hexcolorallocate, $HexColorString, $dieOnInvalid=false, $alpha=false) {
        ...
        if (CI_phpthumb_functions::IsHexColor($HexColorString)) {
            $R = hexdec(substr($HexColorString, 0, 2));
            $G = hexdec(substr($HexColorString, 2, 2));
            $B = hexdec(substr($HexColorString, 4, 2));
            return CI_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg_hexcolorallocate, $R, $G, $B, $alpha);
        }
        if ($dieOnInvalid) {
    ...
    function GrayscalePixel($OriginalPixel) {
        $gray = CI_phpthumb_functions::GrayscaleValue($OriginalPixel['red'], $OriginalPixel['green'], $OriginalPixel['blue']);
    ...
    function ImageCreateFunction($x_size, $y_size) {
        ...
        if (CI_phpthumb_functions::gd_version() >= 2.0) {
    ...
    function ImageCopyRespectAlpha(&$dst_im, &$src_im, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, $opacity_pct=100) {
        ...
        for ($x = $src_x; $x < $src_w; $x++) {
            for ($y = $src_y; $y < $src_h; $y++) {
                $RealPixel    = CI_phpthumb_functions::GetPixelColor($dst_im, $dst_x + $x, $dst_y + $y);
                $OverlayPixel = CI_phpthumb_functions::GetPixelColor($src_im, $x, $y);
                $alphapct = $OverlayPixel['alpha'] / 127;
                $overlaypct = (1 - $alphapct) * $opacipct;

                $newcolor = CI_phpthumb_functions::ImageColorAllocateAlphaSafe(
                    $dst_im,
    ...
    function SafeExec($command) {
        ...
        if (empty($AllowedExecFunctions)) {
            ...
            foreach ($AllowedExecFunctions as $key => $value) {
                $AllowedExecFunctions[$key] = !CI_phpthumb_functions::FunctionIsDisabled($key);
            }
        }
    ...
    function filesize_remote($remotefile, $timeout=10) {
        $size = false;
        $url = CI_phpthumb_functions::ParseURLbetter($remotefile);
        if ($fp = @fsockopen($url['host'], ($url['port'] ? $url['port'] : 80), $errno, $errstr, $timeout)) {
            fwrite($fp, 'HEAD '.@$url['path'].@$url['query'].' HTTP/1.0'."\r\n".'Host: '.@$url['host']."\r\n\r\n");
            if (CI_phpthumb_functions::version_compare_replacement(phpversion(), '4.3.0', '>=')) {
    ...
    function URLreadFsock($host, $file, &$errstr, $successonly=true, $port=80, $timeout=10) {
        if (!function_exists('fsockopen') || CI_phpthumb_functions::FunctionIsDisabled('fsockopen')) {
    ...
    function CleanUpURLencoding($url, $queryseperator='&') {
        ...
        $parse_url = CI_phpthumb_functions::ParseURLbetter($url);
    ...
    function SafeURLread($url, &$error, $timeout=10, $followredirects=true) {
        ...
        $parsed_url = CI_phpthumb_functions::ParseURLbetter($url);
        $alreadyLookedAtURLs[trim($url)] = true;

        while (true) {
            $tryagain = false;
            $rawData = CI_phpthumb_functions::URLreadFsock(@$parsed_url['host'], @$parsed_url['path'].'?'.@$parsed_url['query'], $errstr, true, (@$parsed_url['port'] ? @$parsed_url['port'] : 80), $timeout);
            if (eregi('302 [a-z ]+; Location\\: (http.*)', $errstr, $matches)) {
                ...
                if (!@$alreadyLookedAtURLs[$matches[1]]) {
                    ...
                    $parsed_url = CI_phpthumb_functions::ParseURLbetter($matches[1]);
                }
            }
            if (!$tryagain) {
                break;
        ...
        if (function_exists('curl_version') && !CI_phpthumb_functions::FunctionIsDisabled('curl_exec')) {
            $ch = curl_init();
            ...
            while (($file = readdir($dirhandle)) !== false) {
                ...
                if (is_file($fullfilename)) {
                    ...
                    switch ($file) {
                        ...
                        default:
                            $AllFiles[] = $fullfilename;
                            $subfiles = CI_phpthumb_functions::GetAllFilesInSubfolders($fullfilename);
    ...
    function SanitizeFilename($filename) {
        ...
        if (CI_phpthumb_functions::version_compare_replacement(phpversion(), '4.1.0', '>=')) {
