...
class CI_phpthumb_bmp {

    function CI_phpthumb_bmp() {
        return true;
    }
    ...
    function GD2BMPstring(&$gd_image) {
        ...
        for ($y = ($imageY - 1); $y >= 0; $y--) {
            $thisline = '';
            for ($x = 0; $x < $imageX; $x++) {
                $argb = CI_phpthumb_functions::GetPixelColor($gd_image, $x, $y);
        ...
        $BITMAPFILEHEADER .= CI_phpthumb_functions::LittleEndian2String($bmpSize, 4); // DWORD   bfSize;
        $BITMAPFILEHEADER .= CI_phpthumb_functions::LittleEndian2String(       0, 2); // WORD    bfReserved1;
        $BITMAPFILEHEADER .= CI_phpthumb_functions::LittleEndian2String(       0, 2); // WORD    bfReserved2;
        $BITMAPFILEHEADER .= CI_phpthumb_functions::LittleEndian2String(      54, 4); // DWORD   bfOffBits;

        // BITMAPINFOHEADER - [40 bytes] http://msdn.microsoft.com/library/en-us/gdi/bitmaps_1rw2.asp
        $BITMAPINFOHEADER  = CI_phpthumb_functions::LittleEndian2String(      40, 4); // DWORD  biSize;
        $BITMAPINFOHEADER .= CI_phpthumb_functions::LittleEndian2String( $imageX, 4); // LONG   biWidth;
        $BITMAPINFOHEADER .= CI_phpthumb_functions::LittleEndian2String( $imageY, 4); // LONG   biHeight;
        $BITMAPINFOHEADER .= CI_phpthumb_functions::LittleEndian2String(       1, 2); // WORD   biPlanes;
        $BITMAPINFOHEADER .= CI_phpthumb_functions::LittleEndian2String(      24, 2); // WORD   biBitCount;
        $BITMAPINFOHEADER .= CI_phpthumb_functions::LittleEndian2String(       0, 4); // DWORD  biCompression;
        $BITMAPINFOHEADER .= CI_phpthumb_functions::LittleEndian2String(       0, 4); // DWORD  biSizeImage;
        $BITMAPINFOHEADER .= CI_phpthumb_functions::LittleEndian2String(    2835, 4); // LONG   biXPelsPerMeter;
        $BITMAPINFOHEADER .= CI_phpthumb_functions::LittleEndian2String(    2835, 4); // LONG   biYPelsPerMeter;
        $BITMAPINFOHEADER .= CI_phpthumb_functions::LittleEndian2String(       0, 4); // DWORD  biClrUsed;
        $BITMAPINFOHEADER .= CI_phpthumb_functions::LittleEndian2String(       0, 4); // DWORD  biClrImportant;
    ...
    function PlotPixelsGD(&$BMPdata, $truecolor=true) {
        ...
        foreach ($BMPdata['data'] as $row => $colarray) {
            if (!CI_phpthumb_functions::FunctionIsDisabled('set_time_limit')) {
                set_time_limit(30);
            }
